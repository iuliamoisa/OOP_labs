...
class Animal {
//fiind private, clasele mostenite din Animal nu le pot accesa
//=> clasele mostenite (ex. Dog) nu vor putea folosi this->name
// ci pot accesa name prin metoda publica GetName()
...
private:
    ...
    static int numOfAnimals;// shares the same value 
//for all objects inside "Animal" class
//if it is changed, it's changed for every Animal
...
Animal::Animal() {
    //default constructor: in caz ca nu exista atribute specificate
    this->name = "";
    this->weight = 0;
    this->height = 0;
    Animal::numOfAnimals++;
...
class Dog : public Animal {
    //INHERITANCE: Dog mostenestetot ce are clasa Animal
private:
...
public:
    ...
    // constructor
    Dog() : Animal() {};
    //initializeaza tot ca deafult constr din clasa Animal
...
Dog::Dog(std::string name, double height,
    ...
    // ia name,height,weight de la animal
    //mai trb initializat doar sound ptc ala nu e in animal
    this->sound = sound;
}
